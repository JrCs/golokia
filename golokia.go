...
/* Jolokia Response for Exec request */
...
/* Internal function to perform http exec request and return JolokiaExecResponse */
func executeExecRequest(url string) (*JolokiaExecResponse, error) {
	resp, respErr := http.Get(url)
	if respErr != nil {
		return nil, respErr
	}
	defer resp.Body.Close()
	var respJ JolokiaExecResponse
	dec := json.NewDecoder(resp.Body)
	if err := dec.Decode(&respJ); err != nil {
		return nil, err
	}
...
/* Return a list of properties (attributes) available for a specific jolokia agent for specified url, domain and bean */
func ExecProperties(service, domain, bean, operation string) ([]interface{}, error) {
	resp, err := executeExecRequest(service + "/jolokia/exec/" + domain + ":" + bean + "/" + operation + "?mimeType=application/json")
	if err != nil {
		return nil, err
	}
	ret := resp.Value
